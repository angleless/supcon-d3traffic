...
class Channelization extends Component {
  ...
  roadBg () {
    ...
    const getPoint = streetInfos.reduce((p, d, i, a) => {
      ...
      const { angle: angleLast } = a[(i - 1 + a.length) % a.length]
      if (i === 0) {
        ...
        return [...p, {
          ...d,
          startLineEnd: { x: endOneX, y: endOneY },
          startLineStart: { x: startOneX, y: startOneY },
          endLineEnd: { x: endTwoX, y: endTwoY },
          endLineStart: { x: startTwoX, y: startTwoY }
        }]
      } else {
        const { endLineStart, endLineEnd } = p[p.length - 1]
        // const isPoint = Math.abs(angleLast.to - angleLast.from) <= 180 ? endLineStart : startLineStart
        const splitStart = splitPoint(endLineEnd, endLineStart, 1.5)
        const lens = pointLenth(endLineEnd, endLineStart) * 0.5
        const { x: xCurrLineNext1, y: yCurrLineNext1 } = polar2rect(splitStart, angleLast.to, length + lens)
        const { x: xCurrLineNext, y: yCurrLineNext } = splitPoint(splitStart, { x: xCurrLineNext1, y: yCurrLineNext1 }, lens / (length + lens))
        const angleRectNext = angleScope(angleLast.to)
        const isMinus = 1
        // if (angleLast.to >= 315 && angleLast.to <= 360) {
        //   isMinus = angleLast.from + 180 >= 315 && angleLast.from + 180 <= 360 ? 1 : -1
        // } else if (angleLast.to > 135 && angleLast.to <= 315) {
        //   isMinus = angleLast.from + 180 >= 135 && angleLast.from + 180 <= 315 && angleLast.to <= angleLast.from + 180 ? -1 : 1
        // } else if (angleLast.to > 0 && angleLast.to <= 135) {
        //   isMinus = 1
        // }
        const newRectxNext1 = angleRectNext ? xCurrLineNext1 : xCurrLineNext1 + widthNext * isMinus
        const newRectyNext1 = angleRectNext ? yCurrLineNext1 + widthNext * isMinus : yCurrLineNext1
        const reatCenterNextX1 = angleRectNext ? xCurrLineNext : xCurrLineNext + widthNext * isMinus
        const reatCenterNextY1 = angleRectNext ? yCurrLineNext + widthNext * isMinus : yCurrLineNext
        return [...p, {
          ...d,
          startLineEnd: { x: xCurrLineNext1, y: yCurrLineNext1 },
          startLineStart: { x: xCurrLineNext, y: yCurrLineNext },
          endLineEnd: { x: newRectxNext1, y: newRectyNext1 },
          endLineStart: { x: reatCenterNextX1, y: reatCenterNextY1 }
        }]
      }
    }, [])
    svg.append('g')
      ...
      .append('path')
      .attr('d', (d, i) => {
        const { startLineEnd, startLineStart, endLineEnd, endLineStart } = d
        // const { angle, length, width, widthNext } = d
        // const { x: endOneX, y: endOneY } = polar2rect({ x: w / 2, y: h / 2 }, angle.from, length)
        // const bufferNum = 30
        // const scale = bufferNum / length
        ...
        // let isMinus = -1
        // if (angle.to >= 315 && angle.to <= 360) {
        //   isMinus = angle.from + 180 >= 315 && angle.from + 180 <= 360 ? 1 : -1
        // } else if (angle.to > 135 && angle.to <= 315) {
        //   isMinus = angle.from + 180 >= 135 && angle.from + 180 <= 315 && angle.to <= angle.from + 180 ? -1 : 1
        // } else if (angle.to > 0 && angle.to <= 135) {
        //   isMinus = 1
        // }
        ...
        return `M ${startLineStart.x} ${startLineStart.y} L ${startLineEnd.x} ${startLineEnd.y} M ${endLineStart.x} ${endLineStart.y} L ${endLineEnd.x} ${endLineEnd.y} `
      })
      .attr('stroke', '#ffffff')
      .attr('fill', 'none')
