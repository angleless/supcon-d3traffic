...
class Channelization extends Component {
  ...
  roadBg () {
    ...
    svg.append('g')
      ...
      .append('path')
      .attr('d', (d, i) => {
        const { angle, length, width } = d
        const { x: xCurrLine, y: yCurrLine } = polar2rect({ x: w / 2, y: h / 2 }, angle.from, length)
        const bufferNum = 30
        const scale = bufferNum / length
        const { x: xNewCurrLine, y: yNewCurrLine } = splitPoint({ x: w / 2, y: h / 2 }, { x: xCurrLine, y: yCurrLine }, scale)
        const angleRect = angleScope(angle.from)
        const newRectx1 = angleRect ? xCurrLine : xCurrLine + width / 2
        const newRecty1 = angleRect ? yCurrLine + width / 2 : yCurrLine
        const newRectx2 = angleRect ? xCurrLine : xCurrLine - width / 2
        const newRecty2 = angleRect ? yCurrLine - width / 2 : yCurrLine
        const reatCenterX1 = angleRect ? xNewCurrLine : xNewCurrLine + width / 2
        const reatCenterY1 = angleRect ? yNewCurrLine + width / 2 : yNewCurrLine
        const reatCenterX2 = angleRect ? xNewCurrLine : xNewCurrLine - width / 2
        const reatCenterY2 = angleRect ? yNewCurrLine - width / 2 : yNewCurrLine
        // const { a: a1, b: b1, c: c1 } = lineAnglePoint(angle.from, { x: newRectx1, y: newRecty1 })

        // next line

        // const { x: xCurrLineNext, y: yCurrLineNext } = polar2rect({ x: w / 2, y: h / 2 }, angle.to, length)
        // const { x: xNewCurrLineNext, y: yNewCurrLineNext } = splitPoint({ x: w / 2, y: h / 2 }, { x: xCurrLineNext, y: yCurrLineNext }, scale)
        // const angleRectNext = angleScope(angle.to)
        // const newRectxNext1 = angleRectNext ? xCurrLineNext : xCurrLineNext + widthNext / 2
        // const newRectyNext1 = angleRectNext ? yCurrLineNext + (widthNext / 2) : yCurrLineNext
        // const newRectxNext2 = angleRectNext ? xCurrLineNext : xCurrLineNext - widthNext / 2
        // const newRectyNext2 = angleRectNext ? yCurrLineNext - widthNext / 2 : yCurrLineNext
        // const reatCenterNextX1 = angleRectNext ? xNewCurrLineNext : xNewCurrLineNext + widthNext / 2
        // const reatCenterNextY1 = angleRectNext ? yNewCurrLineNext + widthNext / 2 : yNewCurrLineNext
        // const reatCenterNextX2 = angleRectNext ? xNewCurrLineNext : xNewCurrLineNext - widthNext / 2
        // const reatCenterNextY2 = angleRectNext ? yNewCurrLineNext - widthNext / 2 : yNewCurrLineNext
        // const nextLinePoint = { x: newRectxNext1, y: newRectyNext1 }
        // const { a: a2, b: b2, c: c2 } = lineAnglePoint(angle.to, nextLinePoint)
        // const { x: xJoint, y: yJoint } = intersectionPoint2({ a1, b1, c1 }, { a2, b2, c2 })
        // console.log(newRectx1, )
        // const { x: xnewCurrLine, y: ynewCurrLine } = coord.math2svg({ x: xCurrLine, y: yCurrLine })
        // const { x: x1, y: y1 } = coord.math2svg({ x: 0, y: 0 })
        // console.log({ x: xnewCurrLine, y: ynewCurrLine }, { x: x1, y: y1 })
        return ` M ${reatCenterX1} ${reatCenterY1} L ${newRectx1} ${newRecty1} M ${reatCenterX2} ${reatCenterY2} L ${newRectx2} ${newRecty2}`
      })
      .attr('stroke', '#ffffff')
      .attr('fill', 'none')
