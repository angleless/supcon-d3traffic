...
function polar2rect ({ x, y }, angle = 0, distance = 100) {
  console.log(fixed(200 + distance * Math.sin(ang2rad(180))))
  return {
...
/**
 * 输入角度，输出角度对应的区间偏x轴还是偏y轴
 * @param {number} { angle }
 * @returns {bealon} true 是偏x轴 false偏y轴
 */
function angleScope (angle) {
  const angleScale = Math.floor(angle / 360)
  return (angle >= angleScale * 360 + 45 && angle <= angleScale * 360 + 135) || (angle >= angleScale * 360 + 225 && angle <= angleScale * 360 + 315)
}
