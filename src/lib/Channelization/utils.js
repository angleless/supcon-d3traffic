...
function caclStreetInfos (aspectData) {
  ...
  const streetsWithNext = tabStreets.reduce(
    (p, v, i, a) => {
      console.log(p, v)
  ...
  const result = streetsWithNext.reduce((p, c, i, a) => {
    ...
    const safePath = 60
    const { angle, angleNext, length, lengthNext, ins, outs, laneWidth, laneWidthNext, nextIns, nextOuts } = c
    const len = ins.length + outs
    const lenNext = nextIns.length + nextOuts
    const totalWidth = laneWidth ? laneWidth * len : 25 * len
    const totalNextWidth = laneWidthNext ? laneWidthNext * lenNext : 25 * lenNext
    ...
    console.log(xCurrLine, yCurrLine)
