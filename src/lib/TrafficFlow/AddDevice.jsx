...
class AddDevice extends Component {
  ...
  componentDidMount () {
    ...
    // 全局body绑定click事件当点击非弹窗框的元素时，弹出框会消失隐藏
  ...
  eventCancel = (e) => {
    // 找到其中的class元素为“editedAddDevice”的为弹出框，否则不是。为弹出框中点击展示，其他元素时点击取消弹出框
  ...
  deleteDevicesSvg = () => (
    <svg name='editedAddDevice' viewBox='0 0 1024 1024' id='DELETE-DEVICES' width='16' height='16'>
      <path name='editedAddDevice' d='M61.44 382.016v-128h896v128zM541.44 382.016h128v448h-128z' fill='#737373' />
      <path name='editedAddDevice' d='M285.44 382.016v448h448v-448h-448z m-128-128h704v704h-704v-704z' fill='#737373' />
      <path name='editedAddDevice' d='M349.44 382.016h128v448h-128zM381.44 254.016h256v-64h-256v64z m384-192v320h-512v-320h512z' fill='#737373' />
    </svg>
  )
  ...
  render () {
    ...
    return (
      <div className={`device-modal ${this.state.showModals ? '' : 'no-box'}`} name='editedAddDevice' rotateorigin={rotateorigin} origin={origin} coordinate={coordinate} rotateadjust={rotateadjust} id={id}>
        {
          this.state.showModals &&
            <div className='add-device-modal' name='editedAddDevice'>
              <div className='add-device-warp' name='editedAddDevice'>
                <div className='add-device-modal-content' name='editedAddDevice'>
                  <div
                    className='add-device-svg'
                    name='editedAddDevice'
                    onClick={() => {
                      this.props.deviceClick('CAMERA')
                      this.setState({
                        showModals: false
                      })
                    }}
                  ...
                  <div
                    className='add-device-svg center'
                    name='editedAddDevice'
                    onClick={() => {
                      this.props.deviceClick('COIL')
                      this.setState({
                        showModals: false
                      })
                    }}
                  ...
                  <div
                    ...
                    name='editedAddDevice'
                    onClick={() => {
                      this.props.deviceClick('COMPDETECTOR')
                      this.setState({
                        showModals: false
                      })
                    }}
                  ...
                  <div
                    ...
                    name='editedAddDevice'
                    onClick={() => {
                      this.props.deviceClick('DELETE-DEVICES')
                      this.setState({
                        showModals: false
                      })
                    }}
                  >
                    {this.deleteDevicesSvg()}
                    <span>
                      <span>删除设备</span>
                    </span>
                  </div>
                </div>
                <i className='arrows' name='editedAddDevice' />
              </div>
            </div>
        }
        {/* {
          !this.state.showModals &&
            <div className='add-device' rotateOrigin='road' origin='road[1]{2}' coordinate={{ x: 50, y: 50 }} rotateAdjust='0' id='addDevices'>
              <div className='edited-center'>
                <i className='edited-h' />
                <i className='edited-c' />
              </div>
            </div>
        } */}
      </div>
