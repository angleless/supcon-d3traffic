...
class Plugin extends Component {
  ...
  // 中心plugin定位
  crossOrigin () {
    const { children: { props: { coordinate: { x, y }, rotateorigin, rotateadjust } }, index, w: width, h: height, propData, left, top } = this.props
    ...
    const translateX = width * x1 - innerWidth / 2 + x + left
    const translateY = height * y1 - innerHeight / 2 + y + top
    const rotateParams = rotateorigin === 'pluginCenter' ? `translate(${translateX} ${translateY}) rotate(${rotateadjust} ${innerWidth / 2} ${innerHeight / 2})` : ''
  ...
  insPlugin () {
    ...
    if (streetInfos.length !== 0) {
      ...
      const startLine1 = splitPoint(start, end, (outs + 1 + j) / turnNo)
      const startLine2 = splitPoint(start, end, (outs + 1 + j + 1) / turnNo)
      const widths = pointLenth(startLine1, startLine2)
      const len = pointLenth(start, inflectionStart)
  ...
  roadOrigin () {
    ...
    if (streetInfos.length !== 0 && streetInfos[roadId]) {
      ...
      if (rotateorigin === 'pluginCenter') { // 以插件的中心点进行旋转
  ...
  calculationProps () {
     ...
     * 1. plugin
     * 2. cross
     * 3. out_roadId_index_middle_center    out 出口道 roadId 道路下标  index 车道下标 middle/start/end 车道长度位置 center/left/right 车道宽度位置
     * 4. ins_roadId_index_middle_center    ins 进口道 roadId 道路下标  index 车道下标 middle/start/end 车道长度位置 center/left/right 车道宽度位置
     * 5. road_roadId_index   road 道路 roadId 道路下标  index 车道数/进口道数/出口道数
    */
    switch (origin) {
      case 'plugin':
        this.pluginOrigin()
